...
**Tech Stack:** 空项目默认采用 C# / WPF、Windows Forms NotifyIcon、Windows 官方 Win32 API 和独立控制台 Helper。目标框架固定为 `net10.0-windows`（.NET 10 LTS）；`global.json` 锁定实施时机器上的 .NET 10 SDK 波段。V1 发布目标为 Windows 11 x64。
...
6. 不因为“未来可能扩展”引入没有当前收益的抽象层。允许为查询、自身 Trim、Helper 启动/等待提供 internal 接口或委托，仅供测试注入；禁止插件系统、DI 容器或通用 Feature 框架。
7. 不默默扩大内存清理范围。
...
2026-09-09 工作区复查：当前已是 Git 仓库，默认分支 `main`，远程 `origin` 为 `https://github.com/lqiaoqing/desktop_tools.git`。已跟踪文件为 `README.md`、`docs/dev-guidelines.md`、`.gitignore` 和本计划。没有源码、Solution、Tray、日志或测试。上述能力均应标记为不存在，不得报告已复用。实施前重新检查，若出现真实工程，优先复用并更新映射。
...
主程序始终从 `AppContext.BaseDirectory\helper\DesktopTools.Helper.exe` 启动 Helper。Debug / Release 构建必须通过 Helper 工程 PostBuild（或主工程等价目标）把 Helper 可执行文件及其依赖复制到该相对路径；`dotnet run`、F5 与发布目录使用同一布局，禁止开发态另写一套查找逻辑。

实施步骤：先固定 SDK 和测试依赖版本，再建立可启动/关闭的托盘骨架及单实例，随后按下列阶段加入查询、清理和 Helper。默认每用户、每会话一个主程序，使用会话内命名互斥量 `Local\DesktopTools.SingleInstance`；第二次启动直接退出，不增加激活 IPC。多个登录会话不属于此单实例范围。

首次启动显示主窗口。关闭窗口采用隐藏窗口，托盘可再次打开同一窗口；只有托盘明确退出才结束应用。内存信息在启动、窗口显示和手动刷新时读取，V1 不新增后台轮询定时器。
...
CurrentSnapshot 允许 unavailable，并附查询错误及是否过期。清理结果中的 Before/After 是不可变的历史证据，不是第二份实时状态。状态发布统一在 UI Dispatcher 上完成；查询请求使用递增序号，较旧请求不得覆盖较新采样。清理的 Before / After 查询占用同一套序号。清理开始时不取消已发出的普通刷新，但序号更旧的刷新结果必须丢弃，不得覆盖本次 Before / After 或更新后的 CurrentSnapshot。清理期间不再启动新的普通刷新；清理使用自己的 Before / After 查询，不等待进行中的刷新完成。
...
系统内存读取必须集中到现有最合适的 Platform / System / Memory 模块中。V1 固定调用 `GlobalMemoryStatusEx`：`ullTotalPhys` → TotalPhysicalMemory，`ullAvailPhys` → AvailablePhysicalMemory，`dwMemoryLoad` → MemoryLoad。不使用 WMI 或 PerformanceCounter 作为权威来源。
...
- 本空项目使用独立 Helper EXE；主程序 asInvoker，Helper 清单同样 asInvoker，由主程序按次 `runas` 提升，不安装服务，不使用 requireAdministrator 清单以免重复或异常 UAC。
- 主程序只启动固定绝对路径 `Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "helper", "DesktopTools.Helper.exe"))`，不通过 PATH、当前工作目录或用户配置寻找程序。Helper 不动态加载用户指定 DLL、脚本或插件。
- Helper 命令行协议固定为：

```text
DesktopTools.Helper.exe cache-flush --parent-pid <uint32> --parent-start <int64>
```

  第一个位置参数必须是 `cache-flush`（当前唯一支持的操作）。`--parent-pid` 为主程序 PID；`--parent-start` 为主程序创建时间的 UTC FileTime 整数（与 `GetProcessTimes` 的 `ftCreationTime` 一致）。参数缺失、无法解析、未知开关或多余位置参数时退出码 10。拒绝任意命令、目标进程、输出路径、缓存大小参数。父进程标识只能用于等待/查询，不能用于操作该进程。
- 父进程监测：`OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION | SYNCHRONIZE, FALSE, pid)`，再用 `GetProcessTimes` 核对创建时间。不得请求 `PROCESS_ALL_ACCESS`、`PROCESS_TERMINATE` 或 `PROCESS_VM_*`。无法打开、创建时间不匹配或等待句柄无效时退出码 11，不执行缓存操作。以另一管理员账号提升也必须覆盖此行为。
- 不使用临时结果文件、标准输出重定向或命名管道。主程序持有本次提升产生的真实进程句柄，通过该句柄等待并读取退出码；不能以 PID 搜索到的其他进程或日志内容判定成功。
- 固定退出码协议：0 表示缓存 API 成功；10 参数/协议不支持；11 父进程身份或存活验证失败；12 特权准备失败；13 缓存 API 失败；14 Helper 内部异常。其他退出码均视为异常终止；没有有效进程句柄不能报告成功。看门狗因超时自退使用非 0 且不同于 10–14 的退出码（固定为 15），主程序按结果未知处理。
...
- Helper 启动后等待预算为 30 秒。该预算用于检测挂死，不是“清理必须在 30 秒内产生可见内存收益”的 SLA。超时显示 ResultUnavailable 并进入收尾，不释放门闩供再次清理，也不自动重试。
- Helper 自身设置独立 30 秒看门狗并监测父进程；父进程退出、监测失败或预算到期时终止自身，避免普通权限主程序依赖终止高权限进程。看门狗只管理本次 Helper 寿命，不是自动清理定时器。`SetSystemFileCacheSize((SIZE_T)-1, (SIZE_T)-1, 0)` 通常在发起刷新后返回；若调用本身阻塞超过预算，按挂死处理。
...
空项目日志位置：主程序写入 `%LocalAppData%\DesktopTools\logs\app.log`；Helper 写入其所在用户的 `%LocalAppData%\DesktopTools\logs\helper.log`。两进程不得写同一文件。目录创建或写入失败只记录为日志失败，不得改变清理结果。
...
自动测试使用可替换的查询、Trim、Helper 启动/等待依赖（internal 接口或委托即可），不在单元测试中触发 UAC 或真实缓存清理。这些接缝不是插件系统，不得为此引入 DI 容器或跨功能服务定位器：
